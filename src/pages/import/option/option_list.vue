  ...
  <AppLayout>
    <MainContentWarp>
      <div class="p-2 surface-section flex-auto">
        <div class="p-1">
          <router-link to="/import/importOption">
            <Button
              label="นำเข้าข้อมูลตัวเลือกเสริม"
      ...
      >
        ...
        <p class="line-height-3 p-0 m-0">
          ต้องการลบข้อมูลกลุ่ม {{ del_data.code }} ~ {{ del_data.name1 }} ใช่หรือไม่
        </p>
        <template #footer>
...
function getOptionImportList() {
  MasterdataService.getOptionImport()
    .then((res) => {
      console.log(res);
      if (res.success) {
...
function confirmDel() {
  ...
  console.log(deldata);
  MasterdataService.delOptionImport(deldata)
    .then((res) => {
      console.log(res);
      if (res.success) {
        getOptionImportList();
