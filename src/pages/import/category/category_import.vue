...
function ImportFile() {
  ...
  reader.onload = function (e) {
    ...
    for (let index = 0; index < results.length; index++) {
      details.push({
        index: index,
        code: results[index].รหัสกลุ่ม,
        name1: results[index].ชื่อ1,
        name2: results[index].ชื่อ2,
        parentguid: results[index].รหัสกลุ่มหลัก,
        ordernumber: results[index].เรียงลำดับ,
      });
    }
...
function DownloadExampleExcel() {
  ...
  XLSX.utils.book_append_sheet(wb, Example, "ImportCategoryExample");
  XLSX.writeFile(wb, "ImportCategoryExample.xlsx");
}
...
function delRow(data) {
  ...
  for (let index = 0; index < import_detail.value.length; index++) {
    details.push({
      ...
      parentguid: import_detail.value[index].parentguid,
      ordernumber: import_detail.value[index].ordernumber,
    });
  }
...
function onSave() {
  ...
  if (checkdata != false) {
    ...
    import_detail.value.forEach((ele) => {
      post_data.push({
        code: localStorage.shopid + ele.code,
        name1: ele.name1,
        name2: ele.name2,
        parentguid: ele.parentguid != "" ? localStorage.shopid + ele.parentguid : "",
        categoryguid: localStorage.shopid + ele.code,
        order: parseInt(ele.ordernumber),
      });
    });
    console.log(post_data);
    MasterdataService.importCategory(post_data)
      .then((res) => {
        console.log(res);
        if (res.success) {
          setTimeout(async () => {
            router.push("/import/listCategory");
          }, 1000);
        }
      })
      .catch((err) => {
