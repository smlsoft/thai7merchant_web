  ...
  <AppLayout>
    <MainContentWarp>
      <div class="p-2 surface-section flex-auto">
        <div class="p-1">
          <router-link to="/restaurant/kitchen/kitchen_import">
            <Button
              label="นำเข้าข้อมูลครัว"
      ...
      >
        ...
        <p class="line-height-3 p-0 m-0">
          ต้องการลบข้อมูลสินค้า {{ del_data.CODE }} ~
      ...
      >
        ...
        <p class="line-height-3 p-0 m-0">
          ต้องการลบข้อมูลสินค้าที่เลือก ใช่หรือไม่
        </p>
        <template #footer>
          <div class="border-top-1 surface-border pt-3">
            <Button
...
function confirmDelSelect() {
  ...
  if (deldata.length > 0) {
    console.log(deldata);
    RestaurantService.delCategoryImport(deldata)
      .then((res) => {
        console.log(res);
        if (res.success) {
          getKitchenList();
...
function getKitchenList() {
  RestaurantService.getKitchenList()
    .then((res) => {
      console.log(res);
      if (res.success) {
        data_detail.value = res.data;
        if (res.data.zone != undefined) {
          data_detail.value.zones =
            res.data.zone.code != undefined ? res.data.zone.code : "";
        }
