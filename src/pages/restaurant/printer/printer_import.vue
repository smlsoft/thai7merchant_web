        ...
        <div class="grid">
          <div class="col-12">
            ...
            >
              ...
              <Column field="address" header="ไอพี" :sortable="true"></Column>
              <Column
                field="type"
                header="ประเภท"
                :sortable="true"
...
function ImportFile() {
  ...
  reader.onload = function (e) {
    ...
    for (let index = 0; index < results.length; index++) {
      details.push({
        index: index,
        code: results[index].รหัสเครื่องพิมพ์,
        name1: results[index].ชื่อเครื่องพิมพ์,
        address: results[index].ไอพี,
        type: results[index].ประเภท,
...
function DownloadExampleExcel() {
  ...
  XLSX.utils.book_append_sheet(wb, Example, "ImportPrinterExample");
  XLSX.writeFile(wb, "ImportPrinterExample.xlsx");
}
...
function delRow(data) {
  ...
  for (let index = 0; index < import_detail.value.length; index++) {
    details.push({
      ...
      address: import_detail.value[index].address,
      type: import_detail.value[index].type,
...
function onSave() {
  ...
  if (checkdata != false) {
    ...
    RestaurantService.importPrinter(post_data)
      .then((res) => {
        console.log(res);
        if (res.success) {
          setTimeout(async () => {
            router.push("/restaurant/printer/printer_list");
          }, 1000);
        }
      })
      .catch((err) => {
